...
tms_DLPFC = readtable('/Users/xianqliu/Library/CloudStorage/OneDrive-UniversityofIowa/1_TMS_iEEG/3_ProcessedData_Backup/site_specificity_amy/TEP/archived/TEP_amy_tms_LDLPFC.csv');
tms_Parietal = readtable('/Users/xianqliu/Library/CloudStorage/OneDrive-UniversityofIowa/1_TMS_iEEG/3_ProcessedData_Backup/site_specificity_amy/TEP/archived/TEP_amy_tms_RParietal.csv');

condition = tms_DLPFC.Time > 25;  
tms_DLPFC = tms_DLPFC(condition, :);

condition = tms_Parietal.Time > 25;  
tms_Parietal = tms_Parietal(condition, :);
...
if laterality == 'L'
    condition = ismember(tms_DLPFC.DKT, ['Left-Amygdala']);
    tms_DLPFC = tms_DLPFC(condition, :);
    condition = ismember(tms_Parietal.DKT, ['Left-Amygdala']);
    tms_Parietal = tms_Parietal(condition, :);
    if subdivision == 0 % medial
        condition = ismember(tms_DLPFC.is_lateral, [0]);
        tms_DLPFC = tms_DLPFC(condition, :);
        condition = ismember(tms_Parietal.is_lateral, [0]);
        tms_Parietal = tms_Parietal(condition, :);
    elseif subdivision == 1 % lateral
        condition = ismember(tms_DLPFC.is_lateral, [1]);
        tms_DLPFC = tms_DLPFC(condition, :);
        condition = ismember(tms_Parietal.is_lateral, [1]);
        tms_Parietal = tms_Parietal(condition, :);
    end
elseif laterality == 'R'
    condition = ismember(tms_DLPFC.DKT, ['Right-Amygdala']);
    tms_DLPFC = tms_DLPFC(condition, :);
    condition = ismember(tms_Parietal.DKT, ['Right-Amygdala']);
    tms_Parietal = tms_Parietal(condition, :);
    if subdivision == 0 % medial
        condition = ismember(tms_DLPFC.is_lateral, [0]);
        tms_DLPFC = tms_DLPFC(condition, :);
...
channel_DLPFC = unique(tms_DLPFC.Channel);
channel_Parietal = unique(tms_Parietal.Channel);


target_data = zeros(height(channel_DLPFC), 475);
for subject = 1:height(channel_DLPFC)
    condition = ismember(tms_DLPFC.Channel, channel_DLPFC(subject));
    data_temp = tms_DLPFC(condition, :);
...
for subject = 1:height(channel_Parietal)
    condition = ismember(tms_Parietal.Channel, channel_Parietal(subject));
    data_temp = tms_Parietal(condition, :);
